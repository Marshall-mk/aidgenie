...
def run_seg(img_dir):
    ...
    with col2:
        ...
        st.button(label="Next need annotate", on_click=next_annotate_file, key="seg_next_ann_image")
 
    # Specify canvas parameters in application
    drawing_mode = column_3.selectbox("Drawing tool:", ("point", "freedraw", "line", "rect", "circle", "transform"))
    ...
    with column_1:
        if image is not None:        
            ...
            if canvas_result.image_data is not None:
                st.image(canvas_result.image_data) # width=img_width, use_column_width=True
...
def dataframe_segmentation(path, img_file_name):
    ...
    if 'result_df_seg' not in st.session_state:
        st.session_state['result_df_seg'] = pd.DataFrame.from_dict({'image': st.session_state["files"], 'mask': [None]*len(st.session_state["files"])}).copy()
    st.session_state['result_df_seg'].loc[st.session_state["image_index"], 'mask'] = img_file_name + '_mask.png'
    filtered_df = dataframe_explorer(st.session_state['result_df_seg'], case=False)
    st.dataframe(filtered_df, use_container_width=True)
    file_name = st.text_input('Enter filename for the dataframe',help='Write out the desired file name to save the dataframe.', key="seg_df")
    if st.button("Save frame",key='seg_frame'):
        st.session_state['result_df_seg'].to_csv(f'../data/{file_name}.csv', index=False)
