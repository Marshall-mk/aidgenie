...
def try_out():    # sourcery skip: extract-method, for-append-to-extend, identity-comprehension, list-comprehension, remove-pass-elif, simplify-generator, use-named-expression
    ...
    (
        ...
        [
            "Convert and Store Image Datasets",
            "Dicom Image Explorer and Anonymizer",
            "Anonymize Reports and Generate Tags",
            " Extract Reports from Docs",
            "Research Here/Check Out Papers",
            "Annonate Images and Generate Datasets",
        ]
    )
    ...
    with ConvertStoreTab:
        ...
        if image_path is not None:
            ...
            option = st.selectbox(
                "What format would you like?", ("None", "JPG", "PNG", "H5", "HDF5")
            )
    ...
    with OCRTab:
        (
            ...
            [
                "Convert Pdf Reports to Text Files",
                "Convert Scanned Reports to Text Files",
            ]
        )
        ...
        with PDF2TextTab:
            ...
            if file:
                ...
                filename = st.text_input(
                    "Enter desired filename for the text file: ", key="filename"
                )
                if filename:
        ...
        with SR2TTab:
            ...
            if file:
                ...
                json_filename = st.text_input(
                    "Enter desired filename for the json file: ", key="json_filename"
                )
                if json_filename:
    ...
    with PapersTab:
        ...
        # arxiv pdfs
        topic = st.text_input(
            "Enter the keywords for the topic: ", key="PapersTab_topic"
        )
...
def main():
    with st.sidebar:
        choice = option_menu(
            "Main Menu",
            ["About", "Try out", "Features!"],
            icons=["house", "fire", "minecart-loaded"],
            menu_icon="cast",
            default_index=0,
            styles={
